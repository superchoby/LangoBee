      ...
      for (let i = 0; i < audioFiles.length; ++i) {
        ...
        pronunciationComponents.push(
          <div
            ...
            key={i}
            onClick={async () => { await (new Audio(file)).play() } }
          >
      ...
      {examples.map(example => {
        ...
        return (
          <div
            className='subject-presenter-section-contents subject-presenter-audio-player-container'
            key={kanaExample}
            onClick={async () => { await (new Audio(audioUrl)).play() } }
          >
            <HiSpeakerWave className='subject-presenter-section-play-audio' />
...
interface ContainedRadical {
  character: string
  meaning: string
  type: typeof RADICAL_TYPE
}

interface ContainedOrExampleKanji {
  character: string
  meanings: string[]
...
interface JapaneseExamplesAndContainedSubjectsProps {
  subjects: Array<ContainedRadical | ContainedOrExampleKanji>
}
    ...
    switch (subject.japaneseSubjectType) {
      ...
      case VOCABULARY_TYPE:
        ...
        if (jmdict != null) {
          ...
          const usingCustomMainMeaning = mainMeaningsToUseForVocab.length > 0
          const mainMeaningToUse = usingCustomMainMeaning ? mainMeaningsToUseForVocab[0] : removeParenthesesContent(sense[0].gloss[0].text!)
          ...
          const counterWordSubjectInfo: Array<{ header: string, content: JSX.Element[] }> = []
          ...
          const specialNumberToReading = specialNumbers.reduce<Record<string, { reading: string, explanation?: string }>>((accumulator, { number, reading, explanation }) => (
            {
              ...accumulator,
              [number]: {
                reading,
