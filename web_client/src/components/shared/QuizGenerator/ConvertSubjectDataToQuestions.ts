  ...
  for (const concept of subjectData) {
    if (concept.japaneseSubjectType === KANA_TYPE) {
      ...
      const getAllVocabsMeanings = (jmdictSense: JMDictSense[]) => {
        return jmdictSense.reduce<string[]>((senseAccumulator, { gloss }) => {
          return [
            ...senseAccumulator,
            ...gloss.reduce<string[]>((glossAccumulator, { text }) => {
              return [
                ...glossAccumulator,
                ...(text != null ? [text] : [])
              ]
      ...
      if (jmdict != null) {
        ...
        if (vocabularySubject.customQuestions.length > 0) {
          shuffle(vocabularySubject.customQuestions, { copy: true }).slice(0, 2).forEach((question) => {
            newQuestionsOrder.push({
              questionContents: {
                answers: question.answers.map(answer => ({ answer, distanceToAllow: Math.floor(answer.length * 0.25) })),
                acceptableResponsesButNotWhatLookingFor: [],
                answerIsInJapanese: false,
                question: question.question,
                questionIsOfTypeString: true,
            ...
            questionContents: {
              answers: allMeanings.flatMap((meaning) => {
                const answers = [
                  {
                    answer: meaning,
                    distanceToAllow: Math.floor(meaning.length * 0.25)
                  }
                ]
                  ...
                  answers.push(
                    {
                      ...
                      distanceToAllow: Math.floor(answerWithoutParenthesis.length * 0.25)
                    }
                  )
                }
                ...
                if (specialChars.test(meaning)) {
                  ...
                  answers.push({
                    ...
                    distanceToAllow: Math.floor(answerWithNoSpecialChars.length * 0.25)
                  })
                }
                return answers
              }),
      ...
      const radicalAnswers = [{ answer: radicalSubject.meaning, distanceToAllow: Math.floor(radicalSubject.meaning.length * 0.25) }]
