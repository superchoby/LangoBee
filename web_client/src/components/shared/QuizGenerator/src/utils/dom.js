 ...
 * @private
 */
export function makeOnInput (options, changeCurrentGuess) {
  let prevInput
  ...
  return function onInput ({ target }) {
    if (
      ...
      convertInput(target, mergedConfig, preConfiguredMap, triggers, prevInput, changeCurrentGuess)
    }
  }
}

export function convertInput (target, options, map, triggers, prevInput, changeCurrentGuess) {
  ...
  if (changed) {
    ...
    changeCurrentGuess(target.value)
  }
}
