type ConversionDictionary = Record<string, string>
...
export function convert (convertNumParam: string, dictChoiceUppercase: keyof typeof keyDict): string {
  ...
  if (dictChoice === 'all') {
    const resultList = []
    for (const x of ['kanji', 'hiragana', 'romaji'] as Array<keyof typeof keyDict>) {
...
function doKanjiConvert (convertNum: string) {
  ...
  const numberListConverted = []
  const baseNumber = ['一', '二', '三', '四', '五', '六', '七', '八', '九']
  const linkNumber = ['十', '百', '千', '万', '億']
  ...
  for (const noX of numberList) {
    let count = noX.length
    let result = 0
    let skip = 1
    for (const x of noX.split('').reverse()) {
      let addTo = 0
      skip--
      count--
      if (skip === 1) {
      ...
      if (baseNumber.includes(x)) {
        for (const [y, z] of Object.entries(kanjiDict)) {
          if (z === x) {
            result += parseInt(y)
          }
        }
