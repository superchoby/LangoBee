import React from 'react';

const SvgGrid = () => {
//   const strokePaths = [
//     "M31.01,33c0.88,0.88,2.75,1.82,5.25,1.75c8.62-0.25,20-2.12,29.5-4.25c1.51-0.34,4.62-0.88,6.62-0.5",
//     "M49.76,17.62c0.88,1,1.82,3.26,1.38,5.25c-3.75,16.75-6.25,38.13-5.13,53.63c0.41,5.7,1.88,10.88,3.38,13.62",
//     "M65.63,44.12c0.75,1.12,1.16,4.39,0.5,6.12c-4.62,12.26-11.24,23.76-25.37,35.76c-6.86,5.83-15.88,3.75-16.25-8.38c-0.34-10.87,13.38-23.12,32.38-26.74c12.42-2.37,27,1.38,30.5,12.75c4.05,13.18-3.76,26.37-20.88,30.49",
//   ];
const strokePaths = [
    "M20,24.27c2.89,0.59,5.8,0.73,8.73,0.37c14.14-1.76,34.4-3.66,47.39-4.27c3.06-0.14,6.1,0.19,9.12,0.66",
    "M37.5,14.25c0.81,0.81,1.2,1.95,1.29,2.64c0.96,7.48,0.84,9.48,1.21,13.36",
    "M65.08,10c0.8,1.5,0.64,2.99,0.31,4.38C64.12,19.75,63.75,21,61.58,29",
    "M27.07,37c0.11,0.94,0,1.84-0.33,2.7c-1.36,3.97-6.13,10.4-12.72,16.61",
    "M27.35,42.42c1.36,0,7.57-1.15,10.15-1.55c2.81-0.44,3.66,0.61,2.01,2.86c-2.3,3.13-4.43,6.18-6.88,10.42",
    "M18.18,55.28c0.77,0.77,0.95,1.72,1.1,2.84c0.43,3.3,1.11,9.52,1.65,15.13c0.18,1.9,0.35,3.73,0.48,5.35",
    "M19.65,56.29c6.56-0.87,17.62-2.78,23.63-3.63c2.62-0.37,4.76,0.71,4.49,3.59c-0.32,3.38-1.25,9.02-2.04,14.03c-0.29,1.83-0.56,3.57-0.77,5.08",
    "M32.46,56c0.64,0.64,0.75,1.62,0.75,2.87c0,4.98-0.08,11.95-0.08,15",
    "M21.25,65.87c5.88-0.74,18-2.12,24.25-2.62",
    "M21.77,76.31c7.1-0.56,15.89-1.72,21.93-2.28",
    "M16.78,87.72c0,3.72-2.62,9.22-3.3,10.28",
    "M24.72,85.36c1.64,1.96,3.2,6.83,3.61,9.88",
    "M34.05,84.72c1.82,1.65,4.19,6.52,4.64,9.08",
    "M43.68,83.46c2.16,1.55,5.09,6.14,5.63,8.55",
    "M79.86,32.75c-0.36,1.12-0.73,1.71-1.61,2.38c-3.02,3.2-10.8,7.86-22.39,11.3",
    "M54.81,59.48c2.37,0.4,4.7,0.16,7.09-0.18c6.36-0.92,15.18-1.78,21.85-2.26c2.14-0.34,4.27-0.29,6.39,0.16",
    "M71.92,44.11c0.92,0.92,1.11,1.89,1.11,2.97c0,0.54-0.02,28.34-0.03,42.29c0,3.83,0,6.61,0,7.5",
    "M71.91,58.74c0,1.39-0.8,3.49-1.76,5.2c-4.53,8.1-9.78,17.06-17.4,22.63",
    "M74.25,60.88c6.5,8.75,8.12,11.25,13.54,17.45c2.45,2.8,4.95,5.33,8.21,7"
  ];
  
  const strokeNumbers = [
    { x: 13.85, y: 25.50 },
    { x: 27.50, y: 13.50 },
    { x: 56.50, y: 10.50 },
    { x: 18.50, y: 37.50 },
    { x: 31.50, y: 38.50 },
    { x: 12.50, y: 66.50 },
    { x: 26.50, y: 52.18 },
    { x: 36.50, y: 60.50 },
    { x: 23.50, y: 62.50 },
    { x: 23.50, y: 73.95 },
    { x: 8.50, y: 88.50 },
    { x: 17.50, y: 87.50 },
    { x: 26.50, y: 86.50 },
    { x: 34.25, y: 84.50 },
    { x: 68.50, y: 33.58 },
    { x: 52.50, y: 56.43 },
    { x: 62.50, y: 51.50 },
    { x: 57.50, y: 69.35 },
    { x: 80.50, y: 67.50 }
  ];
  
const grossPath = 'M0,54 L109,54 M54,0 L54,109';
  const gridItems = Array.from({ length: 30 }, (_, i) => (
    <svg key={i} width="109" height="109" viewBox="0 0 109 109" style={{ border: '1px solid #ccc' }}>
        {i + 1 <= strokePaths.length && (
            <React.Fragment key={i}>
                {/* #aaa */}
                <g style={{ fill: 'none', strokeWidth: '5', strokeLinecap: 'round', strokeLinejoin: 'round' }}>
                    {strokePaths.slice(0, i + 1).map((path, idx) => (
                        <path key={idx} d={path} style={{stroke:i === idx ? '#3A3A3A' : '#aaa'}} />
                    ))}
                </g>
                
                <g style={{ fontSize: '12', fill: 'black' }}>
                    <text transform={`matrix(1 0 0 1 ${strokeNumbers[i].x} ${strokeNumbers[i].y})`}>{i + 1}</text>
                </g>
            </React.Fragment>
        )}
        {i < 20 && <path d={grossPath} style={{ stroke: '#ccc', strokeWidth: '1', strokeLinecap: 'square' }} />}
    </svg>
  ));

  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(10, 1fr)', width: '0'}}>
      {gridItems}
    </div>
  );
};

export default SvgGrid;
